...
export default function Portfolio({ setPage }) {
  ...
  return (
    <>
      <PageHero label="Portfolio" title="Work we're proud of" desc="Real projects, real clients, real results. Browse our work across industries and 3D printing technologies." />
      ...
      <section style={{ background: C.forestBg, padding: "60px 24px 88px" }}>
        <div style={{ maxWidth: 1180, margin: "0 auto" }}>
          ...
          <div style={{ marginBottom: 32 }}>
            <p style={{ color: C.muted, fontSize: 11, fontWeight: 600, letterSpacing: 1, textTransform: "uppercase", marginBottom: 10 }}>Industry</p>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 16 }}>
              {industries.map(f => (
                <button key={f} onClick={() => setIndFilter(f)} style={{ padding: "7px 16px", borderRadius: 20, fontSize: 12, cursor: "pointer", transition: "all .2s", background: indFilter === f ? C.accent : "rgba(255,255,255,0.05)", border: `0.5px solid ${indFilter === f ? C.accent : "rgba(255,255,255,0.1)"}`, color: indFilter === f ? C.forestBg : C.muted, fontWeight: indFilter === f ? 600 : 400 }}>
                  {f}
                </button>
              ))}
            </div>
            <p style={{ color: C.muted, fontSize: 11, fontWeight: 600, letterSpacing: 1, textTransform: "uppercase", marginBottom: 10 }}>Technology</p>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              {techs.map(f => (
                <button key={f} onClick={() => setTechFilter(f)} style={{ padding: "7px 16px", borderRadius: 20, fontSize: 12, cursor: "pointer", transition: "all .2s", background: techFilter === f ? "#3B82F6" : "rgba(255,255,255,0.05)", border: `0.5px solid ${techFilter === f ? "#3B82F6" : "rgba(255,255,255,0.1)"}`, color: techFilter === f ? "#fff" : C.muted, fontWeight: techFilter === f ? 600 : 400 }}>
                  {f}
                </button>
              ))}
            </div>
          ...
          <p style={{ color: C.faint, fontSize: 12, marginBottom: 20 }}>{filtered.length} project{filtered.length !== 1 ? "s" : ""} found</p>
            ...
            {filtered.map((p, i) => (
              <Reveal key={p.id} delay={i * 50}>
                <div onClick={() => setSelected(p)}
                  style={{ borderRadius: 14, overflow: "hidden", border: "0.5px solid rgba(255,255,255,0.07)", cursor: "pointer", transition: "all .2s", background: "rgba(255,255,255,0.02)" }}
                  onMouseEnter={e => { e.currentTarget.style.borderColor = p.accent + "60"; e.currentTarget.style.transform = "translateY(-4px)"; }}
                  onMouseLeave={e => { e.currentTarget.style.borderColor = "rgba(255,255,255,0.07)"; e.currentTarget.style.transform = "translateY(0)"; }}>
                  <div style={{ height: 120, background: p.bg, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 40, position: "relative" }}>
                    {p.emoji}
                    <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: 2, background: p.accent }} />
                    <span style={{ position: "absolute", top: 10, right: 10 }}><Badge color={p.accent}>{p.cat}</Badge></span>
                  </div>
                  <div style={{ padding: "14px 16px" }}>
                    <p style={{ color: "#6b9e80", fontSize: 10, fontWeight: 600, letterSpacing: 1, textTransform: "uppercase", marginBottom: 4 }}>{p.industry}</p>
                    <h3 style={{ color: C.white, fontSize: 13, fontWeight: 500, marginBottom: 5, lineHeight: 1.4 }}>{p.title}</h3>
                    <p style={{ color: C.muted, fontSize: 12, lineHeight: 1.6, marginBottom: 10 }}>{p.desc.substring(0, 80)}...</p>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      <span style={{ color: C.muted, fontSize: 11 }}>{p.material} · {p.client}</span>
                      <span style={{ color: C.accent, fontSize: 12, display: "flex", alignItems: "center", gap: 3 }}>View {Icon.arrow}</span>
                    </div>
                  </div>
                </div>
              </Reveal>
          ...
          {filtered.length === 0 && (
            <div style={{ textAlign: "center", padding: "60px 24px" }}>
              ...
              <p style={{ color: C.muted, fontSize: 14 }}>No projects match those filters. Try a different combination.</p>
            </div>
          )}
        </div>
      </section>
      ...
      <section style={{ background: C.sectionAlt, padding: "72px 24px" }}>
        <div style={{ maxWidth: 1180, margin: "0 auto" }}>
          <Reveal><SectionLabel>Client feedback</SectionLabel></Reveal>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(260px,1fr))", gap: 14, marginTop: 32 }}>
            {TESTIMONIALS.slice(0, 3).map((t, i) => (
              <Reveal key={t.name} delay={i * 70}>
                <div style={{ background: "rgba(255,255,255,0.03)", border: "0.5px solid rgba(255,255,255,0.07)", borderRadius: 14, padding: 20 }}>
                  <div style={{ display: "flex", gap: 2, marginBottom: 10 }}>
                    {[...Array(t.rating)].map((_, j) => <span key={j} style={{ color: "#FBBF24", fontSize: 12 }}>{Icon.star}</span>)}
                  </div>
                  <p style={{ color: C.muted, fontSize: 13, lineHeight: 1.7, fontStyle: "italic", marginBottom: 14 }}>"{t.quote}"</p>
                  <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                    <div style={{ width: 30, height: 30, borderRadius: "50%", background: C.brandGreen, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 600, color: C.accent }}>{t.init}</div>
                    <div><p style={{ color: C.white, fontSize: 12, fontWeight: 500 }}>{t.name}</p><p style={{ color: C.faint, fontSize: 11 }}>{t.role}</p></div>
                  </div>
                </div>
              </Reveal>
            ))}
      ...
      <section style={{ background: "#0F2D1A", padding: "72px 24px", textAlign: "center", borderTop: "0.5px solid rgba(34,197,94,0.12)" }}>
        <Reveal>
          ...
          <h2 style={{ color: C.white, fontSize: 26, fontWeight: 500, margin: "14px 0 12px" }}>Ready to be in our portfolio?</h2>
          <p style={{ color: C.muted, fontSize: 14, marginBottom: 24, maxWidth: 400, margin: "0 auto 24px" }}>Let's build something remarkable together.</p>
          <Btn primary onClick={() => setPage("contact")}>{Icon.upload} Start your project</Btn>
        </Reveal>
      </section>

      <CaseStudyModal project={selected} onClose={() => setSelected(null)} setPage={setPage} />
    </>
  );
}
